void roadsetbotanalyzer() {
 TFile* data = new TFile("currentroadsets.root","update"); 
  TTree* datatree = (TTree*)(data->Get("76_bot"));
 // TFile *target = new TFile("toproad.root","RECREATE" );
  TTree *tree = new TTree("toptree","treelibrated tree");

  Float_t BDT_response;
 TBranch *derp =  datatree->Branch("BDT_NIM3_3",&BDT_response,"BDT_NIM3_3/F");
   TMVA::Tools::Instance();
...
//		tree->Branch(arg3 ,&dvar[24-1+i],arg4);
		reader->AddVariable(arg3,&var[i-1]);
//		tree->Branch(arg3 ,&dvar[i-1], arg4);
		datatree->SetBranchAddress(arg3, &dvar[i-1]);
...
for(int i=1; i<17 ; i++){
		...
		arg1.Form(tname);			arg2.Form(tname2);			arg3.Form(bname);			arg4.Form(bname2);
		tree->Branch(arg3 ,&dvar[24+24+17-1+i+17],arg2);
