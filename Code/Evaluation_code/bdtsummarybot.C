//Evaluation Code for Y-Axis bottom
...
//		databack->SetBranchAddress(arg1, &ivar[i-1]);
		databack->SetBranchAddress(arg3, &ivar[i-1]);
...
//		databack->SetBranchAddress(arg1, &ivar[24+24-1+i]);
		databack->SetBranchAddress(arg3, &ivar[24+24-1+i]);
}
for(int i=1; i<17 ; i++){
		char* tname[10];
		char* tname2[10];
...
//		datatree->SetBranchAddress(arg1, &ivar[24+24+17-1+i+17]);
		datatree->SetBranchAddress(arg3, &ivar[24+24+17-1+i+17]);
//		databack->SetBranchAddress(arg1, &ivar[24+24+17-1+i+17]);
		databack->SetBranchAddress(arg3, &ivar[24+24+17-1+i+17]);
}
for(int i=1; i<17 ; i++){
		char* tname[10];
		char* tname2[10];
...
//		databack->SetBranchAddress(arg1, &ivar[24+24+17-1+i+17+17+17]);
		databack->SetBranchAddress(arg3, &ivar[24+24+17-1+i+17+17+17]);
}
/*for(int i=1; i<17 ; i++){
		char* tname[10];
		char* tname2[10];
...
//datatree->SetBranchAddress("dimuon",&matrix);
 ...
 for (Long64_t ievt=0; ievt<(databack->GetEntries());ievt++) {
	...
	c1->Update();
	c1->SaveAs("AllValueResponse.png");
