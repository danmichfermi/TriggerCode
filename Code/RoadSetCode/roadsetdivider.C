void roadsetdivider() {
 TFile *data = new TFile("roadsettop.root","update"); 
...
//  TFile *target = new TFile("toproad.root","RECREATE" );
 // TTree *tree = new TTree("toptree","treelibrated tree");
  ...
  Double_t BDT_response;
	float BDT;
 TBranch *derp =  roadset->Branch("BDT_MATRIX4_2",&BDT,"BDT_MATRIX4_2/F");
   TMVA::Tools::Instance();
  TMVA::Reader *reader = new TMVA::Reader( "V:Color:!Silent" );
	int lol= 2*(23+16+16+16);
	const int n= lol;
	float var[n];
	float dvar[n];
	int entry=0;
...
TH1F * h1= new TH1F("h1","road response distribution",50,-1,1);
