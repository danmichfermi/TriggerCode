...
//TFile *datas= new TFile("BDTMATRIX1v3.root","READ");
/*TTree *signal = (TTree *) datas->Get("signal");
TFile *data= new TFile("realdatasplit.root","READ");
TTree *background = (TTree *) data->Get("background");*/
//TFile *datam= new TFile("N3_MC.root","READ");
//TTree *signal = (TTree *) datam->Get("signal");
//TFile *datam= new TFile("completecombotrees.root","READ");
//TFile *datam = new TFile("withfilter.root","READ");
//TTree *signal = (TTree *) datam->Get("dynim3mat1");
//TFile *datams= new TFile("realdatasplit.root","READ");
//TTree *background = (TTree *) datams->Get("background");*/
//TFile *data= new TFile("BDTtrain.root","READ");
TFile *data= new TFile("BDT_n3mc_low_binary.root","READ");
...
for(int i=1; i<17; i++){
		...
		sprintf(rname,"h4Y1Lr%i",i);
		sprintf(rname2,"h4Y1Lr%i/I",i);
		sprintf(lname,"h4Y1Ll%i",i);
		sprintf(lname2,"h4Y1Ll%i/I",i);
		sprintf(lname3,"h4Y2Ll%i",i);
		sprintf(lname4,"h4Y2Ll%i/I",i);
		sprintf(rname3,"h4Y2Lr%i",i);
		sprintf(rname4,"h4Y2Lr%i/I",i);
...
/*		t1->Branch(arg1 ,&h4td[i],arg2);
		t1->Branch(arg3 ,&h4bd[i],arg4);
		t1->Branch(arg5 ,&h4Y1Lr[i],arg6);
		t1->Branch(arg7 ,&h4Y1Rr[i],arg8);
		t1->Branch(arg9 ,&h4Y2Lr[i],arg10);
		t1->Branch(arg11 ,&h4Y2Rr[i],arg12);
		t2->Branch(arg1 ,&h4td[i],arg2);
		t2->Branch(arg3 ,&h4bd[i],arg4);
		t2->Branch(arg5 ,&h4Y1Lr[i],arg6);
		t2->Branch(arg7 ,&h4Y1Rr[i],arg8);
		t2->Branch(arg9 ,&h4Y2Lr[i],arg10);
		t2->Branch(arg11 ,&h4Y2Rr[i],arg12);*/
	}
