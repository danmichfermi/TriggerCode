...
TBranch *derp =  datatree->Branch("LRtop_v1",&BDT_response,"LRtop_v1/F");
...
for(int i = 1; i < 10; i++)
{
	...
	reader->AddVariable(arg1,&var[i-1+22]);
	reader->AddVariable(arg2,&var[i-1+22+9]);
	datatree->SetBranchAddress(arg1,&ivar[i-1+22]);
	//databack->SetBranchAddress(arg1,&ivar[i-1+22]);
	datatree->SetBranchAddress(arg2,&ivar[i-1+22+9]);
	//databack->SetBranchAddress(arg2,&ivar[i-1+22+9]);
}
...
for(int i=1; i<9; i++)
{
	...
	arg11.Form(rname3);			
	arg12.Form(rname4);
	reader->AddVariable(arg5,&var[i-1+22+18]);
	reader->AddVariable(arg7, &var[i-1+10+18+22]);
	reader->AddVariable(arg9, &var[i-1+20+18+22]);
	reader->AddVariable(arg11, &var[i-1+22+18+24]);
	datatree->SetBranchAddress(arg5,&ivar[i-1+22+18]);
	datatree->SetBranchAddress(arg7,&ivar[i-1+10+18+22]);	
	datatree->SetBranchAddress(arg9,&ivar[i-1+20+18+22]);
	datatree->SetBranchAddress(arg11,&ivar[i-1+22+18+24]);		
...
reader->BookMVA("BDT method","mcn3_mc_n3_gini_125_7.xml");  
